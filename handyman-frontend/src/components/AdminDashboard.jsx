...
const AdminDashboard = () => {
  ...
  const handleBan = async (id, reason = "დარღვევა") => {
    ...
    try {
      ...
      fetchData();
    } catch (err) { alert("შეცდომა ბლოკირებისას"); }
  };
  ...
  const handleDeleteUser = async (id) => {
    if (!window.confirm("გსურთ მომხმარებლის და მისი შეკვეთების სრულად წაშლა?")) return;
    try {
  ...
  const handleCleanup = async () => {
    if (!window.confirm("⚠️ ეს წაშლის ყველა მომხმარებელსა და შეკვეთას! დარწმუნებული ხართ?")) return;
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-6 border-b pb-2">
        {["stats", "users", "jobs"].map((t) => (
          <button key={t} onClick={() => setActiveTab(t)} className={`px-4 py-2 rounded-t-lg ${activeTab === t ? "bg-blue-600 text-white" : "bg-gray-200"}`}>
            {t === "stats" ? "სტატისტიკა" : t === "users" ? "მომხმარებლები" : "შეკვეთები"}
          </button>
        ))}
      ...
      {activeTab === "stats" && stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white p-4 shadow rounded-lg text-center"><p className="text-gray-500">აქტიური შეკვეთა</p><p className="text-2xl font-bold">{stats.activeJobs}</p></div>
          <div className="bg-white p-4 shadow rounded-lg text-center"><p className="text-gray-500">დასრულებული შეკვეთა</p><p className="text-2xl font-bold">{stats.completedJobs}</p></div>
        </div>
      )}
            ...
            <tbody>
              {jobs.map(j => (
                <tr key={j._id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="text-right p-3"><button onClick={() => handleDeleteJob(j._id)} className="bg-red-600 text-white px-2 py-1 rounded">წაშლა</button></td>
                </tr>
              ))}
            </tbody>
          </table>
